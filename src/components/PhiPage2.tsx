...
function awardShare(reference: string) {
  try {
    ...
    return { progressToNextCoin: w.pendingShareCredits, awarded };
  } catch {
    return {};
  }
}
...
export default function PhiPage2() {
  ...
  function openImages() {
    ...
    if (!q) return;
    const tokenId = currentPhiTokenId(q),
      params = new URLSearchParams({ q, token: tokenId });
    location.assign(`${appPath("phi/images")}?${params.toString()}`);
  }
  ...
  const activeTokenId = currentPhiTokenId(query),
    videoParams = new URLSearchParams({ q: query, token: activeTokenId }),
    soundParams = new URLSearchParams({ q: query, token: activeTokenId });
  return (
