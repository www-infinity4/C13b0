...
import { ArrowLeft, Brain, Building2, Plus, Save, UserRound } from "lucide-react";
...
export default function PhiProfile(){
  ...
  async function submit(event:FormEvent){event.preventDefault();const saved=await saveInfinityProfile(profile);setProfile(clone(saved));setStatus("Profile and reusable AI context saved.")}
  if(!ready)return <main className={styles.page}><div className={styles.loading}>Loading profile…</div></main>;
  return <main className={styles.page}><header><a href={appPath("phi")}><ArrowLeft size={18}/> Infinity Phi</a><div><b>Profile</b><small>Identity · AI context · business · workspace</small></div></header><form onSubmit={submit} className={styles.wrap}>
    <section className={styles.hero}><div><small>REUSABLE CONTEXT</small><h1>Tell Infinity Phi who is creating.</h1><p>Save the facts, preferences, terminology, projects and business context you want Phi to know. Search uses it for local relevance, Code uses it to choose capabilities, and Create can use saved identity details in finished artifacts.</p></div><button type="submit"><Save size={17}/> Save profile</button></section>
    ...
    <section className={styles.card}><div className={styles.cardHead}><Brain/><div><h2>About / AI context</h2><p>Anything you want Search, Code, and Create to understand about you before they work.</p></div></div><div className={styles.grid}>
      <label className={styles.wide}>Reusable context<textarea value={profile.contextNotes} onChange={(e)=>person("contextNotes",e.target.value)} placeholder="Examples: preferred terminology, recurring projects, design rules, industries, goals, writing preferences, what I am currently building, assumptions Phi should remember…"/></label>
    </div><p style={{margin:"12px 0 0",color:"#6d7b87",fontSize:".82rem",lineHeight:1.5}}>This context is used locally for Phi routing and relevance. Contact fields are not appended to public web-search queries.</p></section>

    <section className={styles.card}><div className={styles.cardHead}><Building2/><div><h2>Business context</h2><p>Choose which business and website the current Search, Create, or Code work belongs to.</p></div><button type="button" className={styles.secondary} onClick={addBusiness}><Plus size={16}/> Add business</button></div>
