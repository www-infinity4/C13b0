...
async function research(query: string, identity: Identity): Promise<Source[]> {
  // Wikipedia is the primary path because its browser API explicitly supports CORS.
  // Other providers enrich the answer, but they are never allowed to hold the UI open.
...
export default function PhiPage() {
  ...
  async function runSearch(raw: string, currentHistory: HistoryItem[] = []) {
    ...
    // Move to the result page immediately. External APIs never control navigation.
    setPaper(shell);
    setBusy(true);
    setNotice("");
  ...
  useEffect(() => {
    ...
    // Fast local history can inform disambiguation, but IndexedDB is not allowed
    // to block an incoming search anymore.
  ...
  const purpleOrb = { background: "linear-gradient(145deg,#a855f7,#6d28d9)", boxShadow: "0 10px 34px rgba(109,40,217,.24)" };
  const blueTitle = { color: "#1769d2" };
  const blueSearch = { background: "linear-gradient(135deg,#2878dc,#1554ad)", borderColor: "#69a9ff", boxShadow: "0 10px 32px rgba(21,84,173,.22)" };
    ...
    <main className="phi-mode">
      <div className="phi-shell">
        ...
        <section className={paper ? "phi-search-section compact" : "phi-search-section"}>
          {!paper && <>
            <div className="phi-orb" style={purpleOrb}>φ</div>
            <h1 style={blueTitle}>Infinity φ</h1>
            <p>Structured futures from endless results.</p>
          </>}
          <form onSubmit={submit} className="phi-search-box" style={blueSearch}>
            <input value={query} onChange={(e) => setQuery(e.target.value)} aria-label="Research topic" placeholder="Search" />
            <button disabled={busy} aria-label="Search all sources" style={{ background: "#6d28d9" }}>
        ...
        {paper && <article className="phi-results">
          ...
          <div className="phi-result-grid">
            <section className="phi-answer">
              <div className="phi-ai-label"><span className="phi-mini-orb" style={purpleOrb}>φ</span><b>AI Overview</b></div>
              <h1>{paper.title}</h1>
              ...
              <div className="phi-build-card">
                <div><b>Turn this overview into a website</b><p>The complete answer, evidence, and exact subject move into the builder together.</p></div>
                <a href={`./build/?id=${encodeURIComponent(paper.id)}`} style={{ background: "#6d28d9" }}>φ</a>
              </div>
