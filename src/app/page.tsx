...
export default function HomePage() {
  ...
  function search(event: FormEvent<HTMLFormElement>) {
    ...
    const q = query.trim();
    if (!q) return;
    window.location.assign(`${route("/phi/")}?q=${encodeURIComponent(q)}&run=1`);
  }
    ...
    <main style={{minHeight:"100dvh",background:"radial-gradient(circle at 50% 42%,#12375c 0,#082744 28%,#06192d 58%,#040d18 100%)",color:"white",fontFamily:"Arial Black,Arial,Helvetica,sans-serif",padding:"max(22px,env(safe-area-inset-top)) 22px 48px"}}>
      <div style={{maxWidth:760,margin:"0 auto"}}>
        <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",minHeight:64}}>
          <a href={route("/")} style={{color:"white",textDecoration:"none",fontSize:22,fontWeight:900,letterSpacing:6,lineHeight:1}}>INFINITY</a>
          <details style={{position:"relative"}}>
            ...
            <nav style={{position:"absolute",right:0,marginTop:8,width:220,padding:16,borderRadius:18,background:"#fff",color:"#07111f",zIndex:20,boxShadow:"0 20px 60px #0008"}}>
              <a href={route("/phi/")} style={{display:"block",color:"#0a4d87",fontWeight:900,textDecoration:"none"}}>Infinity Phi</a>
            </nav>
          </details>
        </header>
        ...
        <section style={{minHeight:"calc(100dvh - 150px)",display:"flex",flexDirection:"column",justifyContent:"center",textAlign:"center",padding:"4vh 0 10vh"}}>
          <a href={route("/phi/")} aria-label="Open Infinity Phi" style={{width:132,height:132,borderRadius:999,margin:"0 auto 30px",display:"grid",placeItems:"center",background:"#7c3aed",color:"white",textDecoration:"none",boxShadow:"0 0 0 16px #7c3aed18,0 20px 55px #7c3aed55,0 30px 90px #0008",font:"64px Georgia,serif"}}>φ</a>
