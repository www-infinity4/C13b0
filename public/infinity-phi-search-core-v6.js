(() => {
  ...
  if (window.__infinityPhiSearchCoreV6) return;
  window.__infinityPhiSearchCoreV6 = true;
  ...
  function queryVariants(query) {
    ...
    if (!base) return [];
    const out = [base];
    if (CATALOG.test(base)) out.push(`${base} official list dates archive`);
    else if (VIDEO.test(base)) out.push(`${base} official video${SPORTS.test(base) ? ' ESPN FOX Sports' : ''}`);
    else if (NEWS.test(base)) out.push(`${base} official latest coverage`);
  ...
  function parseResults(text, query, engine) {
    ...
    while ((match = pattern.exec(raw)) && out.length < 70) {
      ...
      if (/^(?:www\.)?(?:google\.|bing\.|html\.duckduckgo\.|duckduckgo\.|r\.jina\.ai$)/.test(domain)) continue;
      ...
      const nearby = clean(
        raw.slice(pattern.lastIndex, pattern.lastIndex + 760)
          .replace(/\[[^\]]+\]\([^)]+\)/g, ' ')
          .replace(/^#+\s*/gm, ' '),
        620,
      );
      const id = videoId(url);
      if (!overlap(query, `${title} ${nearby}`)) continue;
      out.push({
        title,
        url,
        domain,
  ...
  function pseudoPage(item, index) {
    ...
    return {
      ...
      extract: `${item.provider} source. ${item.extract || `${item.title}. Source-backed public web result.`}`,
      fullurl: item.url,
  ...
  window.InfinityPhiSearchIntelligence = {
    ...
    searchPublicWeb,
    queryVariants,
    version: '2026-09-15-search-core-v6',
  };
})();
