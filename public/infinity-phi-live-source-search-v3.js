(() => {
  ...
  function parseResults(text, query, engine) {
    ...
    while ((match = pattern.exec(String(text || ''))) && out.length < 42) {
      ...
      if (matchesIntent(query, item)) out.push(item);
    }
    return out;
  }

  async function fetchText(url, ms = 3300) {
  ...
  function queryVariants(query) {
    ...
    if (SPORTS.test(query)) {
      ...
      if (SCHEDULE.test(query)) {
        phrases.push(`${subject} ${year} schedule official`);
        phrases.push(`${subject} ESPN NFL.com FOX Sports CBS Sports`);
        ...
        phrases.push(`${subject} official ESPN FOX Sports CBS Sports`);
      } else {
    ...
    return [...new Set(phrases.map((value) => clean(value, 300)).filter(Boolean))].slice(0, 4);
  }
  ...
  async function strongSearch(query) {
    ...
    unique.forEach((item) => {
      ...
      const exact = clean(`${item.title} ${item.extract}`).toLowerCase().includes(clean(query).toLowerCase()) ? 18 : 0;
    ...
    for (const item of unique) {
      ...
      const cap = wantsVideo && item.provider === 'YouTube' ? 8 : 3;
  ...
  function pseudoPage(item, index) {
    ...
    return {
      ...
      extract: item.extract || `${item.title}. Source-backed web result.`, fullurl: item.url,
  ...
  async function finalFetch(input, init) {
    ...
    if (!query) return priorFetch(input, init);

    const [baseResult, webResult] = await Promise.allSettled([
      priorFetch(input, init),
      strongSearch(query),
    ]);
    if (baseResult.status !== 'fulfilled') throw baseResult.reason;
    const response = baseResult.value;
    ...
    const combined = [];
    const seen = new Set();
    [...webResult.value.map(pseudoPage), ...existing].forEach((page) => {
